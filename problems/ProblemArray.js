 ...
 * Short Answer: Answer, whether or not to accept any caps
 * Multiple Short Answer: Text by boxes, answers, whether or not to accept any caps
 * Code Box: Initial code in the box
 */
function CodeProblem(type, question, additional) {
	this.type = type;
...
//Common strings used in problems
...
problems = [
	[new CodeProblem(sa, 'Create a motor name to reel in a string.',
		...
		new CodeProblem(mc, 'Why is it necessary to mark some wheel motors as "Reversed" in motor set-up? Check all that are correct.', 
				['checkbox', ["To make the robot drive backwords.", "To correct backwords motor mounting.", "To assure left and right motors will move the same way when given a value.", "To let the robot spin in circles."], [1, 2]]),
	...
	[new CodeProblem(mc, 'Which servo would be used for an arm with a 90 degree range of motion?',
		...
		new CodeProblem(msa, 'Type a line of code in each box as specified.',
				[['Declare an int called retracted with the minimum servo value.', 'Assign servo A to the value of retracted.'], ['int retracted = 0;', 'A = retracted;'], false]),
		new CodeProblem(msa, 'The code below declares a global int called servoIncr with a value of 5 and defines a method called incrementServoA() that returns void and increments servo A by servoIncr.' + blanks,
				[['Box 1:', 'Box 2:', "Box 3:"], ['int servoIncr = 5', 'void incrementServoA()', 'A += servoIncr'], false],
				'//Box 1//;\n//Box 2// {\n\t//Box 3//;\n}'),
		new CodeProblem(msa, 'The code below uses servoIncr from the previous problem and defines a method called decrementServoA() that returns void and decrements servo A by servoIncr.' + blanks,
		...
		new CodeProblem(msa, 'The code below declares a global int called servoStop with a value of 125 and defines a method called isAAtStop() that returns a bool of if the value of servo A is greater than or equal to servoStop.' + blanks,
				[['Box 1:', 'Box 2:', 'Box 3:'], ['int servoStop = 125', 'bool isAAtStop()', 'A >= servoStop ||| servoStop <= A'], false]),
		new CodeProblem(sa, "Type a line of code that sets servo A to the max value spinning clockwise.",
				[['A = 0;'], false]),
		new CodeProblem(sa, "Type a line of code that stops the rotation of servo A.",
				[['A = 127;'], false]),
		new CodeProblem(msa, typeLine,
				[['Box 1:', 'Box 2:'], ['bool isAStopped()', 'return A == 127 ||| return 127 == A'], false]),
		new CodeProblem(cb, 'Define a method called changeServoADirection() that returns void and sets servo A to the opposite direction at the same power. Do not use if statements for this.',
	...
	[new CodeProblem(mc, 'Where is it better to have a logic statement security check?',
		...
		new CodeProblem(cb, 'Define a method called setServoA() that accepts an int of a position and sets servo A to it if it is valid. The method returns the bool of if A was set to the input.',
		...
		new CodeProblem(msa, 'The code below uses a call to determineIRPosition() and readIR() and responds to the returned string. The robot’s wheel motors are called leftMotor and rightMotor for the respective wheel side. If the beacon is to the left or right, set the motors to turn in the corresponding direction at half power. Otherwise, set the robot to drive forward at full speed.',
				[['Box 1:', 'Box 2:', 'Box 3:', 'Box 4:'], ['determineIRPosition(readIR())', 'pos == "left" ||| "left" == pos', 'pos == "right" ||| "right" == pos', 'pos == "straight" ||| "straight" == pos'], false]),
